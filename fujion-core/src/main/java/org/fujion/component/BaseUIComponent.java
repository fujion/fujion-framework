...
public abstract class BaseUIComponent extends BaseComponent implements IDisable {
    ...
    public void setContext(Popup context) {
        Popup oldContext = this.context.getReference();
        
        if (!areEqual(context, oldContext)) {
            context.validatePage(getPage());
            this.context.setReference(context);
            propertyChange("context", oldContext, context, false);
    ...
    public void setPopup(Popup popup) {
        Popup oldPopup = this.popup.getReference();
        
        if (!areEqual(popup, oldPopup)) {
            popup.validatePage(getPage());
            this.popup.setReference(popup);
            propertyChange("popup", oldPopup, popup, false);
